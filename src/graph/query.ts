...
export function fieldsNamed(graph: SiteGraph, semanticName: string): FieldNode[] {
...
export function edgeSourcePage(
  ...
  if (graph.pages.some((p) => p.id === edge.sourceNode)) return edge.sourceNode;
...
export function describeLocator(locator: LocatorDefinition): string {
  const label = locator.name ?? locator.label ?? locator.text ?? locator.placeholder;
