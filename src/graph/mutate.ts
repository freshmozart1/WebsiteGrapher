...
export function unregisterVocab(
    ...
    if (!entry)
        throw new VocabularyError(`"${name}" is not registered in ${kind}`);
    if (entry.builtin) {
        throw new VocabularyError(
            `"${name}" is a builtin ${kind} value and cannot be removed`,
        );
    }
    ...
    graph.vocabulary[kind] = graph.vocabulary[kind].filter(
        (e) => e.name !== name,
    );
}
...
function applyApprovalsPatch(graph: SiteGraph, patch: GraphPatch): void {
    ...
    if (patch.resolveApprovals?.length) {
        ...
        graph.pendingApprovals = graph.pendingApprovals.filter(
            (a) => !done.has(a.id),
        );
    }
}
