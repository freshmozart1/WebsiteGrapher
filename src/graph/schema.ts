...
export const LocatorDefinitionSchema = z
    ...
    .refine(
        (l) =>
            Object.values(l).some((v) => typeof v === 'string' && v.length > 0),
...
export const BUILTIN_VOCABULARY: Vocabulary = {
    pageTypes: [
        {
            name: 'home',
            description: 'Site entry point / landing page',
            builtin: true,
        },
        {
            name: 'overview',
            description: 'Lists many items of one kind',
            builtin: true,
        },
        {
            name: 'detail',
            description: 'Shows one item in full',
            builtin: true,
        },
        {
            name: 'form',
            description: 'Primarily collects input from the user',
            builtin: true,
        },
        {
            name: 'search',
            description: 'Search entry or search results',
            builtin: true,
        },
    ],
    componentTypes: [
        {
            name: 'list',
            description: 'Repeating set of comparable items',
            builtin: true,
        },
        {
            name: 'filter',
            description: 'Control that narrows or reorders a list',
            builtin: true,
        },
        {
            name: 'pagination',
            description: 'Control that reveals further items',
            builtin: true,
        },
        {
            name: 'table',
        ...
        {
            name: 'gallery',
            description: 'Repeating set of images or media',
            builtin: true,
        },
        {
            name: 'button',
            description: 'Standalone control that triggers an action',
            builtin: true,
        },
    ],
    fieldNames: [
        {
            name: 'title',
            description: 'Primary name of the item',
            builtin: true,
        },
        {
            name: 'price',
            description: 'Monetary cost of the item',
            builtin: true,
        },
        {
            name: 'description',
            description: 'Prose describing the item',
            builtin: true,
        },
        {
            name: 'image',
            description: 'Primary visual representation of the item',
            builtin: true,
        },
    ],
};
...
export const SiteGraphSchema = SiteGraphShape.superRefine((g, ctx) => {
    ...
    const claim = (id: string, where: string, path: (string | number)[]) => {
        ...
        if (prior)
            fail(path, `duplicate id "${id}" (already used by ${prior})`);
        else seen.set(id, where);
    };
    g.pages.forEach((p, i) => claim(p.id, 'a page', ['pages', i, 'id']));
    g.components.forEach((c, i) =>
        claim(c.id, 'a component', ['components', i, 'id']),
    );
