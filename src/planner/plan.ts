...
export function buildPlan(
    ...
    if (!chosen) {
        ...
        return {
            ok: false,
            question,
            reason: 'The information exists in the graph, but no learned route reaches it.',
            detail: [
...
function resolveTarget(
    ...
    if (target.kind === 'goal') {
        ...
        if (!goal) {
            return {
                gap: {
                    ...
                    suggestion:
                        'List what is known: wgraph graph show <domain>',
                },
            };
        }
        ...
        return {
            candidates: [
                { pageId: goal.targetPage, ...(field ? { field } : {}) },
            ],
        };
    }
