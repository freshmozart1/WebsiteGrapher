...
export function interpretQuestion(
    ...
    for (const goal of graph.goals) {
        ...
        const score =
            (overlap(tokens, goalTokens) / goalTokens.length) * GOAL_WEIGHT;
        if (score > 0) {
    ...
    for (const name of fieldNamesInUse(graph)) {
        ...
        const own =
            (overlap(tokens, nameTokens) / nameTokens.length) *
            FIELD_NAME_WEIGHT;
    ...
    for (const page of graph.pages) {
        ...
        const typeMatch =
            (overlap(tokens, typeTokens) / typeTokens.length) *
            PAGE_TYPE_WEIGHT;
        const urlMatch =
            overlap(tokens, tokenize(page.urlPattern)) > 0
                ? PAGE_URL_WEIGHT
                : 0;
