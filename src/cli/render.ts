...
export function renderGraph(graph: SiteGraph): string {
  ...
  for (const page of graph.pages) {
    ...
    for (const c of graph.components.filter((c) => c.pageId === page.id)) {
      const extra = [
        ...
        .filter(Boolean)
        .join(" ");
      lines.push(`      component ${c.id}  [${c.type}]  ${describeLocator(c.locator)}${extra ? `  ${extra}` : ""}`);
    }
    for (const f of graph.fields.filter((f) => f.pageId === page.id)) {
      const mark = f.verified ? "" : "  (unverified)";
      lines.push(`      field ${f.semanticName}  ${describeLocator(f.locator)}${mark}`);
    }
    for (const d of page.stateDimensions ?? []) {
      lines.push(`      state ${d.kind}  ${d.label ?? d.componentId}  (${d.valueSource})`);
    }
  }
  ...
  if (graph.pendingApprovals.length > 0) {
    lines.push("", `Awaiting your approval (${graph.pendingApprovals.length}):`);
    for (const a of graph.pendingApprovals) {
      lines.push(`  ${a.id}  ${a.description}`);
      lines.push(`      ${a.reason}`);
    }
    lines.push("", `Approve with: wgraph resume ${hostOf(graph.origin)} --approve <id,...>`);
  }
