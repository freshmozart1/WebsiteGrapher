...
export async function observeCommand(argv: string[]): Promise<number> {
  ...
  if (!target) throw new UsageError("wgraph observe <url> [--json] [--fields]");
  ...
  const result = await withPage(
    ...
    async (page) => {
      ...
      const draft = classifyPage(observation.signals, cluster !== undefined);
      const tiers = tierElements(observation.elements, {
        origin: new URL(page.url()).origin,
      });
      const fields = values.fields ? await findFieldCandidates(page) : null;
  ...
  if (observation.screenshot) lines.push("", `Screenshot: ${observation.screenshot}`);
