...
import { domainKey, graphExists, originOf, withGraph } from "../../graph/store.js";
...
export async function analyzeCommand(argv: string[]): Promise<number> {
  ...
  const outcome = await learnSite({
    ...
    ...(values.quiet ? {} : { log: (line: string) => console.error(`  ${line}`) }),
  });

  return await persist(domain, startUrl, outcome, Boolean(values.json), values.force);
}
...
export async function resumeCommand(argv: string[]): Promise<number> {
  ...
  const target = positionals[0];
  if (!target) {
    throw new UsageError('wgraph resume <domain> --approve <id,...> | --approve-all');
  }
...
async function persist(
  ...
  if (json) {
    console.log(JSON.stringify({ graph, log: outcome.log, spent: outcome.spent }, null, 2));
  } else {
...
function summarize(
  graph: { pages: unknown[]; components: unknown[]; edges: unknown[]; fields: unknown[] },
  outcome: LearnOutcome,
  domain: string,
): string {
  const lines = [
  ...
  if (outcome.stoppedBy) {
    lines.push(`Stopped early on the ${outcome.stoppedBy} budget — raise it to learn more.`);
  }
  if (outcome.pending.length > 0) {
    lines.push("", renderPending(outcome.pending, domain));
  } else {
    lines.push("", `Ask it something: wgraph ask ${domain} "How do I find the price?"`);
  }
  return lines.join("\n");
}
...
function budgetsFrom(values: Record<string, unknown>) {
  ...
  if (probes !== undefined && Number.isFinite(probes)) budgets.maxProbes = probes;
  if (seconds !== undefined && Number.isFinite(seconds)) budgets.maxMs = seconds * 1000;
  return budgets;
}
