...
async function readPatchFile(file: string | undefined): Promise<string> {
  if (!file) {
    throw new UsageError("Pass the patch with --file <path>, --file - or --patch '<json>'.");
  }
  if (file === "-") {
    const chunks: Buffer[] = [];
...
async function setEntry(argv: string[]): Promise<number> {
  const domain = requireDomain(argv[0], "wgraph graph set-entry <domain> <pageId>");
  const pageId = argv[1];
  if (!pageId) throw new UsageError("wgraph graph set-entry <domain> <pageId>");
...
export function requireDomain(input: string | undefined, usage: string): string {
