    ...
    switch (sub) {
        case 'list': {
            ...
            if (sessions.length === 0) {
                console.log(
                    'No saved sessions. (No browser is running either — wgraph never leaves one open.)',
                );
                return EXIT.OK;
            }
            for (const s of sessions) {
                console.log(
                    `${s.name}  ${s.currentUrl}  (updated ${s.updatedAt})`,
                );
            }
            return EXIT.OK;
        }
        ...
        default:
            throw new UsageError(
                'Expected: wgraph session list | clear <name>',
            );
    }
}
