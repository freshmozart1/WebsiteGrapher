...
export class Budget {
  ...
  canVisit(): boolean {
    return this.pages < this.limits.maxPages && this.elapsedMs < this.limits.maxMs;
  }

  canProbe(): boolean {
    return this.probes < this.limits.maxProbes && this.elapsedMs < this.limits.maxMs;
  }

  visit(): void {
    if (!this.canVisit()) {
      throw new BudgetExceeded(this.elapsedMs >= this.limits.maxMs ? "maxMs" : "maxPages");
    }
    this.pages++;
  }
  ...
  probe(): void {
    if (!this.canProbe()) {
      throw new BudgetExceeded(this.elapsedMs >= this.limits.maxMs ? "maxMs" : "maxProbes");
    }
    this.probes++;
  }
}
...
export function inScope(scope: Scope, url: string): ScopeVerdict {
  ...
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    return { allowed: false, reason: `the ${parsed.protocol} scheme is out of scope` };
  }
  if (parsed.origin !== scope.origin) {
    return { allowed: false, reason: `${parsed.origin} is a different site` };
  }
  if (!robotsPermits(scope.robots, parsed.pathname)) {
    return { allowed: false, reason: `robots.txt disallows ${parsed.pathname}` };
  }
