...
import { observePage, primaryCluster, type PageObservation } from "./observe.js";
...
const BUILTIN_FIELD_NAMES = new Set(BUILTIN_VOCABULARY.fieldNames.map((e) => e.name));
...
export async function learnSite(options: LearnOptions): Promise<LearnOutcome> {
  ...
  try {
    ...
    const entryClass = classifyPage(entryObs.signals, entryCluster !== undefined);
    say(
    ...
    patch.components!.push({
      ...
      meta: {
        itemLocator: { css: `${cluster.containerCss} > ${cluster.itemCss}` },
    ...
    say(`list: ${cluster.count} x ${cluster.itemCss} in ${cluster.containerCss}`);
    ...
    for (const el of overview.obs.elements) {
      ...
      try {
        ...
        say(`probe ${describe(el)}: ${result.change.kind} — ${result.change.detail}`);
        ...
        if (result.change.kind === "list-changed") {
          ...
          patch.components!.push({
            ...
            type: "filter",
            locator,
            meta: { controlKind: controlKindOf(el.tag, el.type, el.role) },
          });
          stateDimensions.push({
            ...
            componentId: compId,
            valueSource:
              el.tag === "select" ? "options" : el.tag === "input" ? "free-text" : "options",
    ...
    if (samples.length === 0) {
      say("the list items do not link anywhere, so there is no detail page to learn");
      return done();
    }
    ...
    for (const href of samples) {
      ...
      if (!detailClass) {
        detailClass = classifyPage(obs.signals, primaryCluster(obs.clusters) !== undefined);
      }
    ...
    for (const { candidate, verified } of validated) {
      ...
      if (!BUILTIN_FIELD_NAMES.has(candidate.suggestedName)) {
        registerOnce(patch, "fieldNames", candidate.suggestedName, candidate.reason);
      }
      patch.fields!.push({
        id: fieldId,
        pageId: detailId,
        ...
        verified,
        ...(candidate.valueShape ? { valueShape: candidate.valueShape } : {}),
      });
      say(
...
async function findOverview(
  ...
  const candidates = entry.elements
    ...
    .filter((el) => new URL(el.href!).pathname !== new URL(entry.url).pathname)
    .filter((el) => assessRisk(el, { origin: scope.origin }).tier === "safe")
  ...
  for (const el of candidates) {
    ...
    say(`${path} -> ${classification.type} (${classification.reasons.join("; ")})`);
