...
const PAGINATION_WORDS = [
  "next", "previous", "prev", "load more", "show more", "more results",
  "page", "older", "newer", "»", "«", "›", "‹",
];
...
export async function observePage(
  ...
  return {
    ...
    signals: {
      ...
      inputCount: elements.filter((e) => e.tag === "input" || e.tag === "textarea")
        .length,
      ...
      topClusterLinks: top?.clickTargetCss !== null && top?.clickTargetCss !== undefined,
...
function paginationHints(elements: NormalizedElement[]): string[] {
  ...
  for (const el of elements) {
    ...
    const haystack = `${el.text} ${el.ariaLabel ?? ""} ${el.classes.join(" ")}`
      .toLowerCase()
      .trim();
  ...
  return await page.evaluate(() => {
    ...
    return {
      ...
      postForms: forms.filter(
        (f) => (f.getAttribute("method") ?? "get").toLowerCase() === "post",
      ).length,
