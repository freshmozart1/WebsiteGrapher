...
export function classifyChange(
  ...
  if (urlChanged && changedAPageParam(beforeUrl, afterUrl) && listChange.differs) {
    return {
      ...base,
      ...listChange.flags,
      kind: "pagination",
  ...
  if (before.domHash !== after.domHash || before.interactiveCount !== after.interactiveCount) {
    return {
      ...base,
      kind: "layout-only",
...
function changedAPageParam(before: URL | null, after: URL | null): boolean {
  ...
  for (const param of PAGE_PARAMS) {
    if (before.searchParams.get(param) !== after.searchParams.get(param)) return true;
  }
  return false;
}
