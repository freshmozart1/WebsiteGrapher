...
describe("the helpers reach every document the analyzer looks at", () => {
  it("is there on the first page load", async () => {
    const present = await withPage({ url: `${site.url}/index.html` }, (page) =>
      page.evaluate(() => typeof window.__wgraph?.cssPathOf === "function"),
    );
  ...
  it("is still there after navigating away, and after going back", async () => {
    await withPage({ url: `${site.url}/index.html` }, async (page) => {
      ...
      await page.goto(`${site.url}/products.html`, { waitUntil: "domcontentloaded" });
      expect(
        await page.evaluate(() => typeof window.__wgraph?.cssPathOf === "function"),
      ).toBe(true);

      await page.goto(`${site.url}/product/1.html`, { waitUntil: "domcontentloaded" });
      expect(
        await page.evaluate(() => typeof window.__wgraph?.cssPathOf === "function"),
      ).toBe(true);
  ...
  it("keeps out of the page's own view of its globals", async () => {
    const footprint = await withPage({ url: `${site.url}/index.html` }, (page) =>
      page.evaluate(() => ({
        enumerable: Object.keys(window).includes("__wgraph"),
        forIn: (() => {
          for (const key in window) if (key === "__wgraph") return true;
          return false;
        })(),
      })),
    );
...
describe("cssPathOf", () => {
  it("prefers a unique id", async () => {
    const path = await withPage({ url: `${site.url}/products.html` }, (page) =>
      page.evaluate(() => {
        document.body.insertAdjacentHTML("beforeend", `<div id="only-one"></div>`);
        return window.__wgraph.cssPathOf(document.querySelector("#only-one")!);
      }),
    );
  ...
  it("falls back to a positional path that resolves to the element again", async () => {
    const result = await withPage({ url: `${site.url}/products.html` }, (page) =>
      page.evaluate(() => {
        const target = document.querySelectorAll("li.product-card")[2]!;
        const css = target.getAttribute("id") ? null : window.__wgraph.cssPathOf(target);
        return { css, roundTrips: css ? document.querySelector(css) === target : false };
      }),
    );
  ...
  it("uses a test id only when the caller asks for one", async () => {
    const paths = await withPage({ url: `${site.url}/products.html` }, (page) =>
      page.evaluate(() => {
        document.body.insertAdjacentHTML(
          "beforeend",
          `<div><span data-testid="pick-me"></span></div>`,
        );
        const el = document.querySelector('[data-testid="pick-me"]')!;
        return {
          withOption: window.__wgraph.cssPathOf(el, { preferTestId: true }),
...
describe("landmarkOf", () => {
  it("reports an explicit role as the tag it stands for", async () => {
    const found = await withPage({ url: `${site.url}/index.html` }, (page) =>
      page.evaluate(() => {
        document.body.insertAdjacentHTML(
          "beforeend",
          `<div role="navigation"><a id="in-role-nav" href="/x">x</a></div>`,
        );
        return window.__wgraph.landmarkOf(document.querySelector("#in-role-nav")!);
      }),
    );
    ...
    const found = await withPage({ url: `${site.url}/index.html` }, (page) =>
      page.evaluate(() => {
        ...
        return {
          ...
          chromeOnly: window.__wgraph.landmarkOf(el, ["nav", "header", "footer"]),
        };
      }),
    );
...
describe("the shared helpers leave the analyzers' output alone", () => {
  it("still excludes the header nav and keeps the list in <main>", async () => {
    const obs = await withPage({ url: `${site.url}/products.html` }, (page) =>
      observePage(page),
    );
    ...
    const chrome = obs.clusters.filter((c) => c.excluded && c.landmark !== null);
