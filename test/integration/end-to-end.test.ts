...
import { analyzeCommand, resumeCommand } from "../../src/cli/commands/analyze.js";
...
beforeAll(async () => {
  ...
  const spy = vi
    .spyOn(console, "log")
    .mockImplementation((...args: unknown[]) => captured.push(args.join(" ")));
  sharedCode = await analyzeCommand([site.url, "--quiet", "--max-seconds", "90"]);
  spy.mockRestore();
...
afterAll(async () => {
  ...
  for (const dir of temporaryHomes) await rm(dir, { recursive: true, force: true });
});
...
async function relearnInFreshHome(): Promise<{ code: number; domain: string }> {
  ...
  const code = await analyzeCommand([site.url, "--quiet", "--max-seconds", "90"]);
...
describe("learning the fixture site end to end", () => {
  it("builds a graph with the three page kinds and the route between them", async () => {
    ...
    const toDetail = graph.edges.find((e) => e.targetNode === "page-detail");
  ...
  it("records the pagination mode", async () => {
    const graph = await loadGraph(sharedDomain);
    const pagination = graph.components.find((c) => c.type === "pagination");
  ...
  it("stores no page content anywhere in the graph", async () => {
    ...
    expect(serialized).not.toContain("A small shop that exists only to be analyzed");
  });
...
describe("answering from the stored graph", () => {
  it("answers the price question without opening a browser", async () => {
    const code = await askCommand([sharedDomain, "How do I find the price of a product?"]);
  ...
  it("answers a question about a field name it had to invent", async () => {
    expect(await planCommand([sharedDomain, "--field", "availability"])).toBe(EXIT.OK);
    expect(printed()).toMatch(/availability is on the detail page/);
  });

  it("reports a gap for something it never learned", async () => {
    expect(await planCommand([sharedDomain, "--field", "shipping-weight"])).toBe(
      EXIT.GAP,
    );
  });
});
...
describe("the approval checkpoint", () => {
  ...
  it("clears the queue once approved, without adding a no-op as a filter", async () => {
    ...
    const code = await resumeCommand([domain, "--approve", ids.join(","), "--quiet"]);
    expect(code).toBe(EXIT.OK);
