...
describe('domainKey', () => {
    it('reduces a URL to its host', () => {
        expect(
            domainKey('https://books.toscrape.com/catalogue/x/index.html'),
        ).toBe('books.toscrape.com');
    });
...
describe('originOf', () => {
    it('drops path and query', () => {
        expect(originOf('https://example.com/a/b?c=1#d')).toBe(
            'https://example.com',
        );
    });
...
describe('save and load', () => {
    ...
    it('reports a missing graph with the command that would create it', async () => {
        await expect(loadGraph('nowhere.example')).rejects.toThrow(
            GraphNotFoundError,
        );
        await expect(loadGraph('nowhere.example')).rejects.toThrow(
            /wgraph analyze/,
    ...
    it('rejects a graph written by a newer wgraph', async () => {
        ...
        await expect(loadGraph('books.toscrape.com')).rejects.toThrow(
            /newer wgraph/,
        );
    });
...
describe('withGraph', () => {
    ...
    it('persists the mutation', async () => {
        ...
        expect((await loadGraph('books.toscrape.com')).entryPageId).toBe(
            'page-overview',
        );
    });
    ...
    it('serialises concurrent writers instead of dropping one', async () => {
        ...
        const names = (
            await loadGraph('books.toscrape.com')
        ).vocabulary.fieldNames.map((e) => e.name);
