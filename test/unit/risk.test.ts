...
describe('blocked', () => {
    ...
    it('refuses anything inside a POST form', () => {
        expect(
            tier(el({ text: 'Go', form: { method: 'post', action: '/x' } })),
        ).toBe('blocked');
    });
    ...
    it.each(['password', 'email', 'tel', 'file'])(
        'refuses a %s input',
        (type) => {
            expect(tier(el({ tag: 'input', type, role: 'textbox' }))).toBe(
                'blocked',
            );
        },
    );
    ...
    it('refuses a field whose name suggests payment details', () => {
        expect(
            tier(el({ tag: 'input', type: 'text', name: 'cardNumber' })),
        ).toBe('blocked');
    });
    ...
    it('refuses a link that leaves the site', () => {
        ...
        expect(assessRisk(link, { origin: ORIGIN }).reason).toContain(
            'elsewhere.example',
        );
    });
    ...
    it('refuses a non-http scheme', () => {
        expect(tier(el({ tag: 'a', href: 'mailto:hi@shop.example' }))).toBe(
            'blocked',
        );
    });
});
...
describe('safe', () => {
    ...
    it('allows a link into a product detail page, which is how detail pages get learned', () => {
        const link = el({
            tag: 'a',
            role: 'link',
            text: 'The Silent Harbour',
            href: `${ORIGIN}/product/1.html`,
        });
        expect(tier(link)).toBe('safe');
    });
    ...
    it('allows a search input', () => {
        expect(
            tier(el({ tag: 'input', type: 'search', role: 'searchbox' })),
        ).toBe('safe');
    });
...
describe('confirm', () => {
    ...
    it('asks about a bare text input', () => {
        expect(tier(el({ tag: 'input', type: 'text', role: 'textbox' }))).toBe(
            'confirm',
        );
    });

    it('gives a reason a person could act on', () => {
        const assessment = assessRisk(el({ text: 'Newsletter' }), {
            origin: ORIGIN,
        });
