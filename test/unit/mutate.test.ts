...
describe("registerVocab", () => {
  it("adds an unknown value once", () => {
    ...
    expect(registerVocab(g, "pageTypes", "checkout-step", "One purchase step")).toEqual(
      { added: true },
    );
  ...
  it("marks custom values so builtins stay distinguishable", () => {
    const g = emptyGraph("https://example.com");
    registerVocab(g, "fieldNames", "star-rating", "Stars out of five");
    expect(g.vocabulary.fieldNames.find((e) => e.name === "star-rating")?.builtin).toBe(
      false,
    );
    expect(g.vocabulary.fieldNames.find((e) => e.name === "price")?.builtin).toBe(true);
  });
});
...
describe("unregisterVocab", () => {
  it("refuses to remove a builtin, so nothing already written breaks", () => {
    ...
    expect(() => unregisterVocab(g, "fieldNames", "price")).toThrow(/builtin/);
  });
...
describe("usage reporting", () => {
  ...
  it("reports every kind with its usage, for `graph show --vocabulary`", () => {
    ...
    expect(fields.entries.find((e) => e.name === "star-rating")).toMatchObject({
      inUse: 1,
      builtin: false,
    });
  });
...
describe("applyPatch", () => {
  ...
  it("keeps a page's component list in step automatically", () => {
    const g = emptyGraph("https://shop.example");
    applyPatch(g, {
      pages: [{ id: "page-list", type: "overview", urlPattern: "/items", entry: true }],
      components: [
        {
          id: "comp-list",
          pageId: "page-list",
  ...
  it("generates readable ids when none are given", () => {
    ...
    applyPatch(g, {
      components: [
        { pageId: "page-overview", type: "list", locator: { css: "ul" } },
      ],
    });
  ...
  it("registers vocabulary in the same patch that uses it", () => {
    ...
    applyPatch(g, {
      vocabulary: {
        pageTypes: [{ name: "checkout-step", description: "One purchase step" }],
      },
      pages: [{ id: "page-pay", type: "checkout-step", urlPattern: "/pay", entry: true }],
    });
    expect(() => parseGraph(g)).not.toThrow();
  });
  ...
  it("defaults a new field to unverified", () => {
    const g = bookshopGraph();
    applyPatch(g, {
      vocabulary: { fieldNames: [{ name: "isbn", description: "Book number" }] },
      fields: [
        { pageId: "page-detail", semanticName: "isbn", locator: { css: "#isbn" } },
      ],
    });
